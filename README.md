...
`wwparrecdl/wwparrecdl.py`. The code provided in this file is intentionally rather
...
to create a structure where when we type `import wwparrecdl as sb` in an
interactive Python session, the classes and functions defined inside of the
`wwparrecdl.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the wwparrecdl module, the main classes defined at the bottom of
...
Either way, you can create a `wwparrecdl/data` folder in which you can
...
(typically, something like our `wwparrecdl/tests/test_wwparrecdl.py`). Within each
...
have one such test in `wwparrecdl/tests/test_wwparrecdl.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/wwparrecdl`),
and type:
...
will see a message such as:
    ...
    wwparrecdl/tests/test_wwparrecdl.py .F...
    ...
    wwparrecdl/tests/test_wwparrecdl.py:49: AssertionError
...
To document `wwparrecdl` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/wwparrecdl/blob/master/doc/_static/RTD-advanced-conf.png)

 http://wwparrecdl.readthedocs.org/en/latest/
...
A `wwparrecdl/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/wwparrecdl) for the software. This
...
`wwparrecdl` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `wwparrecdl`, we use the
...
[here](https://github.com/uwescience/wwparrecdl/blob/master/scripts/Figure1.ipynb).
...
### Using `wwparrecdl` as a template
...
button on the front page of the "wwparrecdl" repository.
...
`wwparrecdl/wwparrecdl.py` to be called `smallish/smallish.py`

	git mv wwparrecdl smallish
	git mv smallish/wwparrecdl.py smallish/smallish.py
	git mv smallish/tests/test_wwparrecdl.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `wwparrecdl` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `wwparrecdl` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `wwparrecdl` in them. Type
the following to see where all these files are:

	git grep wwparrecdl

You can replace `wwparrecdl` for `smallish` quickly with:

	git grep -l 'wwparrecdl' | xargs sed -i 's/wwparrecdl/smallish/g'
...
Edit all the mentions of `wwparrecdl` in the `wwparrecdl/__init__.py` file, and in
the `wwparrecdl/version.py` file as well.
